<header class="p-1 md:p-2 w-full z-10 fixed bg-primary-100 dark:bg-primary-800 border-b-2 dark:border-secondary-50">
  <nav class="navbar container flex justify-between mx-auto min-w-full">
    ...
    <ul class="items-stretch space-y-8 mt-20 mb-8 md:mb-0 md:mt-0 hidden -ml-8 md:ml-auto md:space-y-0 md:space-x-3 md:flex peer-checked:block">
      @foreach ($page->siteSections->keys() as $item)
        <li class="flex items-center place-content-center pb-2 md:pb-0 px-4 border-b-2 dark:border-primary-100">
          <x-animated-container
            class="js-nav-item {{ $loop->index === 0 ? 'text-secondary-900 font-semibold dark:text-secondary-50 -translate-y-1 scale-110' : '' }}"
    ...
    <x-animated-container tag="label" for="hbr" class="
      ...
      peer-checked:text-secondary-900
      dark:peer-checked:text-secondary-50
